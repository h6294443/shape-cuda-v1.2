...
int pos2doppler( struct par_t *par, struct photo_t *photo, double orbit_xoff,
		...
		int body, int set, int frm, int v)
{
	...
	static double fit_overflow[MAXOVERFLOW];
	any_overflow = 0;
            ...
            outside the spectrum limits.                                 */
				...
				if (in_bounds) {
					...
					for (idop=idop1; idop<=idop2; idop++) {
						k = MIN( idop - idop_min, MAXBINS);
						fit_contribution = amp * dop_contribution[k];
						fit_overflow[idop+idop0] += fit_contribution;
						if (par->action == MAP && par->map_mode == MAPMODE_DELDOP)
	...
	if (any_overflow) {
		...
		for (j=j1; j<=j2; j++) {
			if (fit_overflow[j] != 0.0) {
				if (par->speckle)
					variance = sdev_sq + lookfact*fit_overflow[j]*fit_overflow[j];
				frame->overflow_o2 += 1.0;
				frame->overflow_m2 += fit_overflow[j]*fit_overflow[j]/variance;
				frame->overflow_xsec += fit_overflow[j];
				frame->overflow_dopmean += (j - idop0)*fit_overflow[j];
			}
		}
