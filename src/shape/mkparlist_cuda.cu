...
__device__ int mpl_nfpar, mpl_nsets;
__device__ int p;

__global__ void mpl_get_nfpar_krnl(struct par_t *dpar, struct
		dat_t *ddat) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0) {
		mpl_nfpar = dpar->nfpar;
		p = -1;
		mpl_nsets = ddat->nsets;
	}
}
		...
		double *fparabstol, int *fpartype, double **fpntr)
{
	...
	checkErrorAfterKernelLaunch("mpl_get_nfpar_krnl (mkparlist_cuda.cu)");
	gpuErrchk(cudaMemcpyFromSymbol(&nfpar, mpl_nfpar, sizeof(int), 0,
