...
__global__ void pos2doppler_radar_parameters_streams_f_krnl(
		struct par_t *dpar,
		struct dat_t *ddat,
		struct dopfrm_t **frame,
		struct pos_t **pos,
...
__global__ void pos2doppler_pixel_streams_f_krnl(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		struct pos_t **pos,
...
__global__ void pos2doppler_finish_streams_f_krnl(
		struct par_t *dpar,
		struct dat_t *ddat,
		struct dopfrm_t **frame,
		float4 *dop,
...
__host__ int pos2doppler_cuda_streams_f(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		struct pos_t **pos,
		...
		cudaStream_t *pds_stream)
{
	...
	struct dopfrm_t **frame;
	float *dopshift;
	float2 *doplim, *axay, *xyincr, orbit_xyoff;
	float3 *w;
	float4 *dop;
	int4 *xylim, host_xylim[nframes];
	...
	for (f=0; f<nframes; f++) {
		/* Loop through all pixels and calculate contributed power */
		pos2doppler_pixel_streams_f_krnl<<<BLK[f],THD,0, pds_stream[f]>>>(dpar,dmod,ddat,
				pos, frame, dop, axay, doplim, xyincr, ndop, idop0, dopshift,
				xspan, set, nframes, nThreads[f], body, orbit_xyoff, f);

		/* Launch the single-thread kernel to finish up Doppler calculations */
		pos2doppler_finish_streams_f_krnl<<<1,1,0,pds_stream[f]>>>(dpar,ddat,frame,
				dop, doplim, dopshift, idop0, ndop, set, nframes, f, badradararr);
	}
	/* Check for errors in the kernel launches & copy the badradar flag back */
	checkErrorAfterKernelLaunch("pos2doppler_finish_krnl, line ");
