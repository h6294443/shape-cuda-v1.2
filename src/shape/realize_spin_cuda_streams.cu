...
__global__ void add_offsets_to_euler_streams2_krnl(struct mod_t *dmod,
		struct dat_t *ddat, double3 *angle_omega_save, int s)
{
	/* Single-threaded kernel */
	/*	angle_omega_save[0].x,y,z = original anglesave[3]
...
__global__ void add_offsets_to_euler_streams2_f_krnl(struct mod_t *dmod,
...
__global__ void realize_spin_dop_streams2_f_krnl(struct mod_t *dmod, struct dat_t *ddat,
...
__global__ void realize_spin_deldop_streams2_f_krnl(struct mod_t *dmod, struct dat_t *ddat,
		struct par_t *dpar, int nframes, int s, int k)
{
	/* nview-threaded kernel */
	int f = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void realize_spin_poset_streams2_f_krnl(struct mod_t *dmod, struct dat_t *ddat,
		struct par_t *dpar, int nframes, int s, int k)
{
	/* nview-threaded kernel */
	int f = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void realize_spin_lghtcrv_streams_f_krnl(struct mod_t *dmod, struct dat_t *ddat,
...
__global__ void update_spin_angle_streams_f_krnl(struct mod_t *dmod,
...
__host__ void realize_spin_cuda_streams2(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		unsigned char *htype,
		...
		cudaStream_t *rs_stream)
{
	...
	for (s=0; s<nsets; s++) {
		 ...
		 * angles to be restored later.          */
		/* Launch kernel do add angle offsets to Euler angles.  Three threads total */
		add_offsets_to_euler_streams2_krnl<<<1,1>>>(dmod,ddat,angle_omega_save,s);
		checkErrorAfterKernelLaunch("add_offsets_to_euler_streams_krnl");
...
__host__ void realize_spin_cuda_streams2f(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		unsigned char *htype,
		...
		cudaStream_t *rs_stream)
{
	...
	for (s=0; s<nsets; s++) {
		 ...
		 * angles to be restored later.          */
		/* Launch kernel do add angle offsets to Euler angles.  Three threads total */
		add_offsets_to_euler_streams2_f_krnl<<<1,1>>>(dmod,ddat,angle_omega_save,s);
		...
		case DOPPLER:
			...
			for (k=0; k<nviews[s]; k++)
				realize_spin_dop_streams2_f_krnl<<<BLK,THD,0,rs_stream[k]>>>(dmod,
		...
		case DELAY:
			...
			for (k=0; k<nviews[s]; k++)
				realize_spin_deldop_streams2_f_krnl<<<BLK,THD,0,rs_stream[k]>>>(
		...
		case POS:
			...
			for (k=0; k<nviews[s]; k++)
				realize_spin_poset_streams2_f_krnl<<<BLK,THD,0,rs_stream[k]>>>(
						dmod, ddat, dpar, nframes[s], s, k);
			checkErrorAfterKernelLaunch("realize_spin_poset_streams_krnl");
		...
		case LGHTCRV:
			...
			realize_spin_lghtcrv_streams_f_krnl<<<BLK,THD>>>(
		...
		/* Final kernel launch in realize_spin_cuda */
		update_spin_angle_streams_f_krnl<<<1,1>>>(dmod, angle_omega_save);
