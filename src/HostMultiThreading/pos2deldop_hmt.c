        ...
        if (in_bounds) {
            ...
            for (idel=idel1; idel<=idel2; idel++)
              for (idop=idop1; idop<=idop2; idop++) {
                ...
                fit_contribution = amp * del_contribution[idel-idel_min]
                                       * dop_contribution[k];
                deldop->fit_overflow[idel+idel0][idop+idop0] += fit_contribution;
                if (par->action == MAP && par->map_mode == MAPMODE_DELDOP)
...
//  double sum =0.0;
  ...
  if (any_overflow) {
    ...
    for (i=i1; i<=i2; i++)
      for (j=j1; j<=j2; j++) {
        if (deldop->fit_overflow[i][j] != 0.0) {
          if (par->speckle)
            variance = sdev_sq + lookfact*deldop->fit_overflow[i][j]*deldop->fit_overflow[i][j];
//          sum += deldop->fit_overflow[i][j];
//          printf("set %i frame %i fit_overflow[%i][%i], %3.8g\n", set, frm, i, j, fit_overflow[i][j]);
          frame->overflow_o2 += 1.0;
          frame->overflow_m2 += deldop->fit_overflow[i][j]*deldop->fit_overflow[i][j]/variance;
          frame->overflow_xsec += deldop->fit_overflow[i][j];
          frame->overflow_delmean += (i - idel0)*deldop->fit_overflow[i][j];
          frame->overflow_dopmean += (j - idop0)*deldop->fit_overflow[i][j];
        }
      }
