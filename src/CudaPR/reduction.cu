...
__global__ void brightness_interm_mgpu_krnl(double *d_odata, double *half_frame,
		int hf) {
	/* Single-threaded intermediate kernel that saves the reduction results into
	 * a GPU array.	 */
	if (threadIdx.x == 0)
		half_frame[hf] = d_odata[0];
}
__global__ void set_lghtcrv_values_mgpu_krnl(struct dat_t *ddat, int s,
		double *d_results,	int nframes) {
	/* nframes-threaded kernel */
	int i = blockIdx.x * blockDim.x + threadIdx.x + 1;

	if (i <= nframes)
		ddat->set[s].desc.lghtcrv.y[i] = d_results[i-1];
}
...
__global__ void deldop_xsec_mgpu_intermediate_krnl(struct dat_t *ddat, float *d_odata,
		float *dsum_rad_xsec, int s, int f) {
	/* Single-threaded kernel but for dual-gpu use.  GPU0 handles even frames
	 * and GPU1 handles odd frames.  Each GPU will fill out half arrays (index hf)
	 */
	float deldop_cross_section = 0.0;
	if (threadIdx.x==0) {
		deldop_cross_section = __double2float_rn(ddat->set[s].desc.deldop.frame[f].overflow_xsec);
		deldop_cross_section += d_odata[0];
		deldop_cross_section *= ddat->set[s].desc.deldop.frame[f].cal.val;
		atomicAdd(&dsum_rad_xsec[0], (__double2float_rn(ddat->set[s].desc.deldop.frame[f].weight) *
				deldop_cross_section));
	}
}
__global__ void deldop_xsec_mgpu_intermediate2_krnl(struct dat_t *ddat, float *d_in,
		float *d_out, int s, int hf, int oddflg) {
	/* Single-threaded kernel but for dual-gpu use.  GPU0 handles even frames
	 * and GPU1 handles odd frames.  Each GPU will fill out half arrays (index hf)
	 */
	...
	if (threadIdx.x==0) {
		f = 2*hf + oddflg;
		d_out[hf] = ((__double2float_rn(ddat->set[s].desc.deldop.frame[f].overflow_xsec)) +
					d_in[0]);
		d_out[hf] = d_out[hf] * ddat->set[s].desc.deldop.frame[f].cal.val;
		d_out[hf] = __double2float_rn(ddat->set[s].desc.deldop.frame[f].weight) * d_out[hf];
				//				deldop_cross_section
//		atomicAdd(&dsum_rad_xsec[0], (__double2float_rn(ddat->set[s].desc.deldop.frame[f].weight) *
//				deldop_cross_section));
	}
...
__host__ void sum_brightness_mgpu(
		struct dat_t *ddat,
		struct pos_t **pos0,
		struct pos_t **pos1,
		int nfrm_alloc,
	...
	/* This dual-GPU version assigns odd frames to GPU0 and even frames to
	 * GPU1 (opposite from radar frames due to the lightcurve offset). Each
	 * GPU uses its own memory space and streams. Each frame reduction result
	 * is written to a nfrm_half0/nfrm_half1-sized device array.  After
	 * completion of all frames, these two half-arrays are then recombined and
	 * written to to lghtcrv->y[nfrm_half0+nfrm_half1].
	 *
	 * Assumptions made for this streams version:
	 * 	- All pos in the set have the same parameters (i.e., size)
	 *
	...
	int numThreads = 0;
	double *d_odata0, *d_odata1, *d_odata2, *d_odata3, *d_odata4, *d_odata5,
		   *d_odata6, *d_odata7, *d_odata8, *d_odata9, *d_results0,*d_results1,
		   *d_results;
	...
	/* Find number of blocks & threads needed for reduction call */
	/* NOTE: This assumes all frames/pos are the same size!      */
	xblock_ythread = getNumBlocksAndThreads(size, maxBlocks, THD.x);
	numBlocks = xblock_ythread.x;
	numThreads = xblock_ythread.y;
	dim3 dimBlock(numThreads, 1, 1);
	dim3 dimGrid(numBlocks, 1, 1);
	size_t arrsz = sizeof(double) * numBlocks;
	size_t h0sz = sizeof(double) * nfrm_half0;
	size_t h1sz = sizeof(double) * nfrm_half1;

	/* Host allocations */
	double hresults0[nfrm_half0], hresults1[nfrm_half1], hresults[nfrm_half0+nfrm_half1];
	...
	gpuErrchk(cudaMalloc((void**)&d_odata6, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata8, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_results0, h0sz));
	gpuErrchk(cudaMalloc((void**)&d_results, (h0sz+h1sz)));
	cudaMemsetAsync(d_odata0, 0, arrsz, gpu0_stream[0]);
	cudaMemsetAsync(d_odata2, 0, arrsz, gpu0_stream[1]);
	cudaMemsetAsync(d_odata4, 0, arrsz, gpu0_stream[2]);
	cudaMemsetAsync(d_odata6, 0, arrsz, gpu0_stream[3]);
	cudaMemsetAsync(d_odata8, 0, arrsz, gpu0_stream[4]);
	cudaMemsetAsync(d_results0, 0, h0sz, gpu0_stream[6]);
	...
	gpuErrchk(cudaMalloc((void**)&d_odata7, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata9, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_results1, h1sz));
	cudaMemsetAsync(d_odata1, 0, arrsz, gpu1_stream[0]);
	cudaMemsetAsync(d_odata3, 0, arrsz, gpu1_stream[1]);
	cudaMemsetAsync(d_odata5, 0, arrsz, gpu1_stream[2]);
	cudaMemsetAsync(d_odata7, 0, arrsz, gpu1_stream[3]);
	cudaMemsetAsync(d_odata9, 0, arrsz, gpu1_stream[4]);
	cudaMemsetAsync(d_results1, 0, h1sz, gpu1_stream[7]);
	...
	while (f<nfrm_alloc) {
		...
		gpuErrchk(cudaSetDevice(GPU0));
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu0_stream[hf]>>>
				(pos0, d_odata0, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu0_stream[hf]>>>(d_odata0, d_results0, hf);
		cudaMemsetAsync(d_odata0, 0, arrsz, gpu0_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	if (f>=nfrm_alloc) break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu1_stream[hf]>>>
				(pos1, d_odata1, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu1_stream[hf]>>>(d_odata1, d_results1, hf);
		cudaMemsetAsync(d_odata1, 0, arrsz, gpu1_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU0));
		hf++;	f++;	if (f>=nfrm_alloc)	break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu0_stream[hf]>>>
				(pos0, d_odata2, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu0_stream[hf]>>>(d_odata2, d_results0, hf);
		cudaMemsetAsync(d_odata2, 0, arrsz, gpu0_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	if (f>=nfrm_alloc) break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu1_stream[hf]>>>
				(pos1, d_odata3, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu1_stream[hf]>>>(d_odata3, d_results1, hf);
		cudaMemsetAsync(d_odata3, 0, arrsz, gpu1_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU0));
		hf++;	f++;	if (f>=nfrm_alloc)	break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu0_stream[hf]>>>
				(pos0, d_odata4, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu0_stream[hf]>>>(d_odata4, d_results0, hf);
		cudaMemsetAsync(d_odata4, 0, arrsz, gpu0_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	if (f>=nfrm_alloc) break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu1_stream[hf]>>>
				(pos1, d_odata5, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu1_stream[hf]>>>(d_odata5, d_results1, hf);
		cudaMemsetAsync(d_odata5, 0, arrsz, gpu1_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU0));
		hf++;	f++;	if (f>=nfrm_alloc)	break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu0_stream[hf]>>>
				(pos0, d_odata6, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu0_stream[hf]>>>(d_odata6, d_results0, hf);
		cudaMemsetAsync(d_odata6, 0, arrsz, gpu0_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	if (f>=nfrm_alloc) break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu1_stream[hf]>>>
				(pos1, d_odata7, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu1_stream[hf]>>>(d_odata7, d_results1, hf);
		cudaMemsetAsync(d_odata7, 0, arrsz, gpu1_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU0));
		hf++;	f++;	if (f>=nfrm_alloc)	break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu0_stream[hf]>>>
				(pos0, d_odata8, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu0_stream[hf]>>>(d_odata8, d_results0, hf);
		cudaMemsetAsync(d_odata8, 0, arrsz, gpu0_stream[hf]);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	if (f>=nfrm_alloc) break;
		device_reduce_block_atomic_kernel_brt<<< dimGrid,dimBlock,0,gpu1_stream[hf]>>>
				(pos1, d_odata9, size, hf, 1);
		brightness_interm_mgpu_krnl<<<1,1,0,gpu1_stream[hf]>>>(d_odata9, d_results1, hf);
		cudaMemsetAsync(d_odata9, 0, arrsz, gpu1_stream[hf]);
	}
	checkErrorAfterKernelLaunch("sum_brightness main loop kernels");

	/* Now copy the half arrays to the host, recombine, and copy into another device array. */
	gpuErrchk(cudaSetDevice(GPU0));
	gpuErrchk(cudaMemcpy(hresults0, d_results0, h0sz, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaSetDevice(GPU1));
	gpuErrchk(cudaMemcpy(hresults1, d_results1, h1sz, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaSetDevice(GPU0));

	/* Now recombine. Note that the frame index starts at 0 here */
	int oddflg = 0;
	hf = 0;
	for (int f=0; f<(nfrm_half0+nfrm_half1); f++) {
		oddflg = f%2;
		if (oddflg==0)
			hresults[f] = hresults0[hf];
		if (oddflg==1) {
			hresults[f] = hresults1[hf];
			hf++;
		}
	}

	/* Now copy this back to a device array and launch kernel to actually
	 * assign these values to lghtcrv->y */
	gpuErrchk(cudaMemcpy(d_results, hresults, (h0sz+h1sz), cudaMemcpyHostToDevice));
	BLK.x = floor((THD.x - 1 + nfrm_alloc)/THD.x);
	set_lghtcrv_values_mgpu_krnl<<<BLK,THD>>>(ddat, set, d_results, nfrm_alloc);
	checkErrorAfterKernelLaunch("set_lghtcrv_values_mgpu_krnl");

	/* GPU0 deallocations */
	gpuErrchk(cudaSetDevice(GPU0));
	gpuErrchk(cudaFree(d_odata0));
	gpuErrchk(cudaFree(d_odata2));
	gpuErrchk(cudaFree(d_odata4));
	gpuErrchk(cudaFree(d_odata6));
	gpuErrchk(cudaFree(d_odata8));
	gpuErrchk(cudaFree(d_results0));
	gpuErrchk(cudaFree(d_results));

	/* GPU1 deallocations */
	gpuErrchk(cudaSetDevice(GPU1));
	gpuErrchk(cudaFree(d_odata1));
	gpuErrchk(cudaFree(d_odata3));
	gpuErrchk(cudaFree(d_odata5));
	gpuErrchk(cudaFree(d_odata7));
	gpuErrchk(cudaFree(d_odata9));
	gpuErrchk(cudaFree(d_results1));
	gpuErrchk(cudaSetDevice(GPU0));
}
...
__host__ float compute_zmax_mgpu(
		struct dat_t *ddat,
		struct pos_t **pos0,
		struct pos_t **pos1,
		int nframes,
	...
	/* Function finds zmax for each deldop frame (pos[f]->z[]) in a streamed
	 * dual-GPU operation.  GPU0 handles even frames and GPU1 handles odd
	 * frames.  Each GPU has its own memory constructs and streams.
	 * This version gets all frames in the set done simultaneously in streams,
	 * which are passed to this function from the parent function.
	 *
	 * Assumptions made for this streams version:
	...
	int f=-1, hf0=-1, hf1=-1, maxThreads = maxThreadsPerBlock;
	int maxBlocks = 2048;
	int numBlocks = 0;
	int numThreads = 0;
	float *d_odata0, *d_odata1, *d_odata2, *d_odata3, *d_odata4, *d_odata5,
		   *d_odata6, *d_odata7, *d_odata_final0, *d_odata_final1, *sum_arr0,
		   *sum_arr1, sum_deldop_zmax=0;
	float2 xblock_ythread;
	dim3 BLK,THD;
	THD.x = maxThreadsPerBlock;
	BLK.x = floor((THD.x - 1 + size)/THD.x);
	...
	dim3 dimGrid(numBlocks, 1, 1);
	size_t arrsz = sizeof(float) * numBlocks;
	size_t nfrm_half0_sz = sizeof(float) * nfrm_half0;
	size_t nfrm_half1_sz = sizeof(float) * nfrm_half1;

	/* Allocate host memory */
	sum_arr0 = (float *)malloc(nfrm_half0_sz);
	sum_arr1 = (float *)malloc(nfrm_half1_sz);

	/* Allocate memory for GPU0*/
	gpuErrchk(cudaSetDevice(GPU0));
	gpuErrchk(cudaMalloc((void**)&d_odata0, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata2, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata4, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata6, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata_final0, nfrm_half0_sz));
	gpuErrchk(cudaMemsetAsync(d_odata_final0, 0, nfrm_half0_sz, gpu0_stream[0]));

	/* Allocate memory for GPU1 */
	gpuErrchk(cudaSetDevice(GPU1));
	gpuErrchk(cudaMalloc((void**)&d_odata1, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata3, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata5, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata7, arrsz));
	gpuErrchk(cudaMalloc((void**)&d_odata_final1, nfrm_half1_sz));
	gpuErrchk(cudaMemsetAsync(d_odata_final1, 0, nfrm_half1_sz, gpu1_stream[0]));

	/* Call reduction - Launch four streams on one GPU, then four on the other
	 * GPU, then repeat  */
	while (f<nframes) {
		gpuErrchk(cudaSetDevice(GPU0));
		f++;	hf0++;
		if ((f >= nframes)||(hf0 >= nfrm_half0)) break;
		cudaMemsetAsync(d_odata0, 0, arrsz, gpu0_stream[hf0]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu0_stream[hf0]>>>
				(pos0, d_odata0, size, hf0);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu0_stream[hf0]>>>
				(ddat, d_odata0, d_odata_final0, set, hf0, 0);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	hf1++;
		if ((f >= nframes)||(hf1 >= nfrm_half1)) break;
		cudaMemsetAsync(d_odata1, 0, arrsz, gpu1_stream[hf1]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu1_stream[hf1]>>>
				(pos1, d_odata1, size, hf1);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu1_stream[hf1]>>>
				(ddat, d_odata1, d_odata_final1, set, hf1, 1);

		gpuErrchk(cudaSetDevice(GPU0));
		f++;	hf0++;
		if ((f >= nframes)||(hf0 >= nfrm_half0)) break;
		cudaMemsetAsync(d_odata2, 0, arrsz, gpu0_stream[hf0]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu0_stream[hf0]>>>
				(pos0, d_odata2, size, hf0);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu0_stream[hf0]>>>
				(ddat, d_odata2, d_odata_final0, set, hf0, 0);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	hf1++;
		if ((f >= nframes)||(hf1 >= nfrm_half1)) break;
		cudaMemsetAsync(d_odata3, 0, arrsz, gpu1_stream[hf1]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu1_stream[hf1]>>>
				(pos1, d_odata3, size, hf1);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu1_stream[hf1]>>>
				(ddat, d_odata3, d_odata_final1, set, hf1, 1);

		gpuErrchk(cudaSetDevice(GPU0));
		f++;	hf0++;
		if ((f >= nframes)||(hf0 >= nfrm_half0)) break;
		cudaMemsetAsync(d_odata4, 0, arrsz, gpu0_stream[hf0]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu0_stream[hf0]>>>
				(pos0, d_odata4, size, hf0);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu0_stream[hf0]>>>
				(ddat, d_odata4, d_odata_final0, set, hf0, 0);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	hf1++;
		if ((f >= nframes)||(hf1 >= nfrm_half1)) break;
		cudaMemsetAsync(d_odata5, 0, arrsz, gpu1_stream[hf1]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu1_stream[hf1]>>>
				(pos1, d_odata5, size, hf1);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu1_stream[hf1]>>>
				(ddat, d_odata5, d_odata_final1, set, hf1, 1);

		gpuErrchk(cudaSetDevice(GPU0));
		f++;	hf0++;
		if ((f >= nframes)||(hf0 >= nfrm_half0)) break;
		cudaMemsetAsync(d_odata6, 0, arrsz, gpu0_stream[hf0]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu0_stream[hf0]>>>
				(pos0, d_odata6, size, hf0);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu0_stream[hf0]>>>
				(ddat, d_odata6, d_odata_final0, set, hf0, 0);

		gpuErrchk(cudaSetDevice(GPU1));
		f++;	hf1++;
		if ((f >= nframes)||(hf1 >= nfrm_half1)) break;
		cudaMemsetAsync(d_odata7, 0, arrsz, gpu1_stream[hf1]);
		device_zmax_block_atomic_kernel<<< dimGrid,dimBlock,0,gpu1_stream[hf1]>>>
				(pos1, d_odata7, size, hf1);
		zmax_mgpu_intermediary_krnl<<<1,1,0,gpu1_stream[hf1]>>>
				(ddat, d_odata7, d_odata_final1, set, hf1, 1);
	}
	checkErrorAfterKernelLaunch("device_mgpu_block_atomic_krnl");

	/* Copy the device arrays from both GPUs back to host arrays, then sum
	 * those up  */
	gpuErrchk(cudaSetDevice(GPU1));
	gpuErrchk(cudaMemcpy(sum_arr1, d_odata_final1, nfrm_half1_sz,
			cudaMemcpyDeviceToHost));
	gpuErrchk(cudaSetDevice(GPU0));
	gpuErrchk(cudaMemcpy(sum_arr0, d_odata_final0, nfrm_half0_sz,
			cudaMemcpyDeviceToHost));

	/* Now just sum up all frames in d_odata_final in a loop, copy results back
	 * to a host array and then return the first element - sum_deldop_zmax */
	/* nfrm_half1 will always be either equal to or one less than nfrm_half0 */
	for (int f=0; f<nfrm_half1; f++)
		sum_deldop_zmax += (sum_arr0[f] + sum_arr1[f]);
	if ((nframes%2)==1)
		sum_deldop_zmax += sum_arr0[nframes-1];
	...
	gpuErrchk(cudaFree(d_odata4));
	gpuErrchk(cudaFree(d_odata6));
	gpuErrchk(cudaFree(d_odata_final0));
	...
	gpuErrchk(cudaFree(d_odata5));
	gpuErrchk(cudaFree(d_odata7));
	gpuErrchk(cudaFree(d_odata_final1));
...
__host__ float compute_deldop_xsec_mgpu(
		struct dat_t *ddat,
		int nframes,
		int nfrm_half0,
		int nfrm_half1,
		...
		cudaStream_t *gpu1_stream) {
	/* Function finds zmax for each deldop frame (pos[f]->z[]) in a streamed
	 * calculation. Eight frames at a time. This is the dual-GPU version where
	 * GPU0 handles even frames and GPU1 handles odd frames. Each GPU has its
	 * own allocated memory and streams that cannot be accessed by the other
	 * GPU (not directly).
	 * This version gets all frames in the set done simultaneously in streams,
	 * which are passed to this function from the parent function.
	 *
	 ...
	 * 	- pos		- all pos structures in current dataset
	 * 	- nframes	- number of frames in dataset
	 * 	- nfrm_half0- number of even frames
	 * 	- nfrm_half1- number of odd frames
	 * 	- size		- size of each pos array, i.e. # of pixels
	 * 	- gpu0_stream	- stream array for GPU0, guaranteed to be as large as the
	 * 				  largest dataset requires and at least 13
	 * 	- gpu1_stream   - same, but for GPU1 */

	/* NOTE AND WARNING!!!  This function produces inconsistent reduction
	 * results (in d_odatax arrays) on all but the first kernel invocations
	 * when run in DEBUG mode.  Everything works fine in RELEASE.	 */
	...
	*sum_arr1, sum_deldop_xsec=0;
	...
	sum_arr0 = (float *)malloc(nfrm_half0_sz);
	sum_arr1 = (float *)malloc(nfrm_half1_sz);
	memset(sum_arr0, 0, nfrm_half0_sz);
	memset(sum_arr1, 0, nfrm_half1_sz);

	/* Allocate memory for GPU0*/
	gpuErrchk(cudaSetDevice(GPU0));
	gpuErrchk(cudaMalloc((void**)&d_odata0, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata2, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata4, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata6, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata_final0, nfrm_half0_sz));
	gpuErrchk(cudaMemsetAsync(d_odata_final0, 0, nfrm_half0_sz, gpu0_stream[0]));

	/* Allocate memory for GPU1 */
	gpuErrchk(cudaSetDevice(GPU1));
	gpuErrchk(cudaMalloc((void**)&d_odata1, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata3, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata5, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata7, sizeof(float)*2));
	gpuErrchk(cudaMalloc((void**)&d_odata_final1, nfrm_half1_sz));
	gpuErrchk(cudaMemsetAsync(d_odata_final1, 0, nfrm_half1_sz, gpu1_stream[0]));
	...
	while (f<nframes) {
		...
		hf0++;	f = 2*hf0;
		if ((f >= nframes)||(hf0 >= nfrm_half0)) break;
		gpuErrchk(cudaMemsetAsync(d_odata0, 0, sizeof(float)*2, gpu0_stream[hf0]));
		device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu0_stream[hf0]>>>
				(ddat, d_odata0, size, f, set);
		deldop_xsec_mgpu_intermediate2_krnl<<<1,1,0,gpu0_stream[hf0]>>>(ddat,
				d_odata0, d_odata_final0, set, hf0, 0);

		hf0++;	f = 2 * hf0;
		if (hf0 < nfrm_half0) {
			gpuErrchk(cudaMemsetAsync(d_odata2, 0, sizeof(float)*2, gpu0_stream[hf0]));
			device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu0_stream[hf0]>>>
					(ddat, d_odata2, size, f, set);
			deldop_xsec_mgpu_intermediate2_krnl<<<1,1 ,0,gpu0_stream[hf0]>>>(ddat,
					d_odata2, d_odata_final0, set, hf0, 0);
		}

		hf0++;	f = 2 * hf0;
		if (hf0 < nfrm_half0) {
			gpuErrchk(cudaMemsetAsync(d_odata4, 0, sizeof(float)*2, gpu0_stream[hf0]));
			device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu0_stream[hf0]>>>
					(ddat, d_odata4, size, f, set);
			deldop_xsec_mgpu_intermediate2_krnl<<<1,1 ,0,gpu0_stream[hf0]>>>(ddat,
					d_odata4, d_odata_final0, set, hf0, 0);
		}

		hf0++;	f = 2 * hf0;
		if (hf0 < nfrm_half0) {
			gpuErrchk(cudaMemsetAsync(d_odata6, 0, sizeof(float)*2, gpu0_stream[hf0]));
			device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu0_stream[hf0]>>>
					(ddat, d_odata6, size, f, set);
			deldop_xsec_mgpu_intermediate2_krnl<<<1,1 ,0,gpu0_stream[hf0]>>>(ddat,
					d_odata6, d_odata_final0, set, hf0, 0);
		}

		gpuErrchk(cudaSetDevice(GPU1));
		hf1++;	f = 2*hf1 + 1;
		if ((f >= nframes)||(hf1 >= nfrm_half1)) break;
		gpuErrchk(cudaMemsetAsync(d_odata1, 0, sizeof(float)*2, gpu1_stream[hf1]));
		device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu1_stream[hf1]>>>
				(ddat, d_odata1, size, f, set);
		deldop_xsec_mgpu_intermediate2_krnl<<<1,1,0,gpu1_stream[hf1]>>>(ddat,
				d_odata1, d_odata_final1, set, hf1, 1);

		hf1++;	f = 2*hf1 + 1;
		if (hf1 < nfrm_half1) {
			gpuErrchk(cudaMemsetAsync(d_odata3, 0, sizeof(float)*2, gpu1_stream[hf1]));
			device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu1_stream[hf1]>>>
					(ddat, d_odata3, size, f, set);
			deldop_xsec_mgpu_intermediate2_krnl<<<1,1,0,gpu1_stream[hf1]>>>(ddat,
					d_odata3, d_odata_final1, set, hf1, 1);
		}

		hf1++;	f = 2*hf1 + 1;
		if (hf1 < nfrm_half1) {
			gpuErrchk(cudaMemsetAsync(d_odata5, 0, sizeof(float)*2, gpu1_stream[hf1]));
			device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu1_stream[hf1]>>>
					(ddat, d_odata5, size, f, set);
			deldop_xsec_mgpu_intermediate2_krnl<<<1,1,0,gpu1_stream[hf1]>>>(ddat,
					d_odata5, d_odata_final1, set, hf1, 1);
		}
		hf1++;	f = 2*hf1 + 1;
		if (hf1 < nfrm_half1) {
			gpuErrchk(cudaMemsetAsync(d_odata7, 0, sizeof(float)*2, gpu1_stream[hf1]));
			device_reduce_block_atomic_kernel_ddf2<<< dimGrid, dimBlock, 0, gpu1_stream[hf1]>>>
					(ddat, d_odata7, size, f, set);
			deldop_xsec_mgpu_intermediate2_krnl<<<1,1,0,gpu1_stream[hf1]>>>(ddat,
					d_odata7, d_odata_final1, set, hf1, 1);
		}
	}
	checkErrorAfterKernelLaunch("reduction loop kernels in deldop_xsec_mgpu");

	/* Now copy the half arrays dsum_rad_xsec from each GPU to its host mirror,
	 * then combine the two halves into the final value that will be returned */
	gpuErrchk(cudaSetDevice(GPU1));
	gpuErrchk(cudaMemcpy(sum_arr1, d_odata_final1, nfrm_half1_sz,
			cudaMemcpyDeviceToHost));
	gpuErrchk(cudaSetDevice(GPU0));
	...
	for (int f=0; f<nfrm_half1; f++)
		sum_deldop_xsec += (sum_arr0[f] + sum_arr1[f]);
	if ((nframes%2)==1)
		sum_deldop_xsec += sum_arr0[nframes-1];
