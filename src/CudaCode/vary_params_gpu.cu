...
__device__ float gpu_deldop_cross_section32=0.0, gpu_doppler_cross_section32=0.0,
		gpu_sum_rad_xsec32=0.0, gpu_sum_cos_subradarlat32=0.0, gpu_sum_deldop_zmax32,
		gpu_sum_opt_brightness32;
...
__global__ void zmax_final_krnl32(float value) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0)
		gpu_sum_deldop_zmax32 += value;
}
__global__ void zmax_final_krnl64(double value) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0)
		gpu_sum_deldop_zmax64 += value;
}

__global__ void xsec_doppler_krnl32(struct dat_t *ddat, float frm_xsec,
		int s, int f) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0) {
		gpu_doppler_cross_section32 = __double2float_rd(ddat->set[s].desc.doppler.frame[f].overflow_xsec);
		gpu_doppler_cross_section32 += frm_xsec;
		gpu_doppler_cross_section32 *= __double2float_rd(ddat->set[s].desc.doppler.frame[f].cal.val);
		gpu_sum_rad_xsec32 += gpu_doppler_cross_section32 *
				__double2float_rd(ddat->set[s].desc.doppler.frame[f].weight);
	}
}
__global__ void xsec_doppler_krnl64(struct dat_t *ddat, double frm_xsec,
		int s, int f) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0) {
...
__global__ void xsec_deldop_krnl32(float value) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0)
		gpu_sum_rad_xsec32 += value;
}
__global__ void xsec_deldop_krnl64(double value) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0)
...
__host__ void vary_params_gpu32(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		int action,
	...
	for (s=0; s<nsets; s++) {
		...
		case DELAY:
			...
			if (hcomp_zmax[s]) {
				...
				zmax_final_krnl32<<<1,1>>>(zmax);
				checkErrorAfterKernelLaunch("zmax_final_krnl32");
			}

			/* Calculate radar cross section for each frame in set */
			xsec[0] = compute_deldop_xsec_gpu32(ddat, hnframes[s], ddsize[0], s, vp_stream);
			xsec_deldop_krnl32<<<1,1>>>(xsec[0]);
			checkErrorAfterKernelLaunch("xsec_deldop_krnl32");
		...
		case DOPPLER:
			...
			for (f=0; f<nfrm_alloc; f++) {
				if (hcomp_xsec[f])
					xsec_doppler_krnl32<<<1,1,0,vp_stream[f]>>>(ddat, xsec[f], s, f);
			}
			if (compute_cosdelta)
...
__host__ void vary_params_gpu64(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		int action,
...
//						posclr_fast_krnl2<<<BLK[f],THD,0,vp_stream[f]>>>(pos, posn, f, FP64, 1);
				...
				if (bistatic_all) {
					...
					checkErrorAfterKernelLaunch("posmask_init_krnl64");
...
void *vary_params_pthread_sub(void *ptr) {
	...
	for (s=0; s<data->nsets; s++) {
		...
		case DOPPLER:
			...
			for (f=0; f<nfrm_alloc; f++) {
				if (hcomp_xsec[f]){
					xsec_doppler_krnl32<<<1,1,0,data->gpu_stream[f]>>>
