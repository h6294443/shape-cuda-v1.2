...
__global__ void vpst_init_krnl(
		struct par_t *dpar,
		struct dat_t *ddat,
		int *compute_zmax,
...
__global__ void zmax_finalize_krnl(float value) {
	...
	if (threadIdx.x == 0) {
		st_sum_deldop_zmax += value;
	}
}
__global__ void compute_xsec_doppler_krnl(struct dat_t *ddat, float frm_xsec,
		int s, int f) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0) {
...
__global__ void compute_xsec_deldop_krnl(float value) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0)
		st_sum_rad_xsec += value;
}
...
__global__ void compute_cosdelta_f_krnl(struct dat_t *ddat, int s, int f) {
	/* Single-threaded kernel */
	if (threadIdx.x == 0) {
		int j, view;
...
__global__ void delay_params_mgpu_krnl(struct par_t *dpar, struct dat_t *ddat,
		...
		int s,
		int f,
		int vf,	/* Virtual frame # */
		int nframes) {
	/* single-threaded, streamed, multi-GPU kernel  */


	if (f < nframes) {
		compute_xsec[vf] = (dpar->vary_radalb != VARY_NONE
				&& ddat->set[s].desc.deldop.frame[f].cal.state == 'c');

		pos[vf] = &ddat->set[s].desc.deldop.frame[f].pos;
		posn[vf] = pos[f]->n;
		ndel[vf] = ddat->set[s].desc.deldop.frame[f].ndel;
		ndop[vf] = ddat->set[s].desc.deldop.frame[f].ndop;
		frame[vf] = &ddat->set[s].desc.deldop.frame[f];
	}
}
...
__global__ void vpst_set_posmtrx_krnl(struct dat_t *ddat,
		struct pos_t **pos,
		unsigned char type,
		int s,
		int f) {
		...
		int max_frames)
{
	...
	dim3 pxBLK,THD,BLKncalc,THD9,BLKpx1, BLK[nfrm_alloc_max], ddBLK[nfrm_alloc_max];
	...
	vpst_init_krnl<<<1,1>>>(dpar, ddat, compute_zmax, compute_cosdelta,
	...
	for (s=0; s<nsets; s++) {
		...
		case DELAY:
			...
			vpst_delay_params_krnl<<<BLK[0],THD>>>(dpar, ddat, pos, ddframe,
			...
			for (f=0; f<nfrm_alloc; f++) {
				...
				if (hcomp_zmax[s] || hcomp_xsec[f]) {
					vpst_set_posmtrx_krnl<<<1,THD9,0,vp_stream[f]>>>(ddat, pos, htype[s],
							s, f);
			...
			compute_xsec_deldop_krnl<<<1,1>>>(xsec[0]);
		...
		case DOPPLER:
			...
			/* Launch nframes-threaded kernel to get all relevant parameters */
			vpst_dop_params_krnl<<<BLK[0],THD>>>(dpar, ddat, pos, dframe,
			...
			for (f=0; f<nfrm_alloc; f++) {
				if (hcomp_xsec[f])
					compute_xsec_doppler_krnl<<<1,1,0,vp_stream[f]>>>(ddat, xsec[f], s, f);
				if (compute_cosdelta)
					compute_cosdelta_krnl<<<1,1,0,vp_stream[f]>>>(ddat, s, f);
			}
			break;
		case POS:
			break;
		case LGHTCRV:
			...
			if (hcomp_brightness[s]) {
				...
				for (f=1; f<nfrm_alloc; f++) {
					vpst_set_posmtrx_krnl<<<1,THD9,0,vp_stream[f-1]>>>(
							ddat, pos, htype[s], s, f);

					posclr_krnl<<<BLK[f],THD, 0, vp_stream[f-1]>>>(pos,posn,f,1);
				}
				checkErrorAfterKernelLaunch("vpst_set_posmtrx_streams_krnl and"
				...
				if (bistatic_all) {
					for (f=1; f<nfrm_alloc; f++) {
						...
						posmask_init_krnl<<<1,1,0,vp_stream[f-1]>>>(pos,
								so, pixels_per_km, f);
		...
		int max_frames)
{
	...
	int c=0, f, s, *compute_brightness, *compute_zmax, *bistatic, bistatic_all,
		*compute_cosdelta, n, ncalc, nx, lghtcrv_bistatic,
		nfrm_alloc, nfrm_alloc_max, nThreads, compute_zmax_flag;
	nfrm_alloc_max = max_frames + 1;
	int nfrm_half = nfrm_alloc_max/2;
	int hcomp_xsec0[nfrm_half], hcomp_xsec1[nfrm_half],
		npxls[nfrm_alloc_max], ddsize[nfrm_alloc_max],
		hndop0[nfrm_half], hndop1[nfrm_half],
		hndel0[nfrm_half], hndel1[nfrm_half],
		hposn0[nfrm_half], hposn1[nfrm_half],
		lc_xspan[nfrm_alloc_max],
		hbistatic[nfrm_alloc_max],
		nThreadspx1[nfrm_alloc_max], hcomp_cosdelta[nsets], hcomp_zmax[nsets+1],
		hcomp_brightness[nsets+1];
	int2 span[nfrm_alloc_max];
	int4 *xylim, hxylim[nfrm_alloc_max];
	...
	gpuErrchk(cudaMalloc((void**)&pos0, sizeof(struct pos_t*) * nfrm_half));
	gpuErrchk(cudaMalloc((void**)&dframe0, sizeof(struct dopfrm_t*)*nfrm_half));
	gpuErrchk(cudaMalloc((void**)&ddframe0, sizeof(struct deldopfrm_t*)*nfrm_half));
	gpuErrchk(cudaMalloc((void**)&posn0, 	   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&ndel0, 	   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&ndop0, 	   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&outbndarr0,   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&compute_xsec0,sizeof(int) * nfrm_alloc_max));

	/* GPU1 allocations */
	gpuErrchk(cudaSetDevice(GPU1));
	gpuErrchk(cudaMalloc((void**)&pos1, sizeof(struct pos_t*) * nfrm_half));
	gpuErrchk(cudaMalloc((void**)&dframe1, sizeof(struct dopfrm_t*)*nfrm_half));
	gpuErrchk(cudaMalloc((void**)&ddframe1, sizeof(struct deldopfrm_t*)*nfrm_half));
	gpuErrchk(cudaMalloc((void**)&posn1, 	   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&ndel1, 	   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&ndop1, 	   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&outbndarr1,   sizeof(int) * nfrm_alloc_max));
	gpuErrchk(cudaMalloc((void**)&compute_xsec1,sizeof(int) * nfrm_alloc_max));
	...
	for (s=0; s<nsets; s++) {
		...
		case DELAY:
			...
			hf = 0;	/* Virtual frame index for the device copies of half size */
			for (int f=0; f<nfrm_alloc; f++) {
				gpuErrchk(cudaSetDevice(GPU0));
				delay_params_mgpu_krnl<<<1,1,0,gpu0_stream[f]>>>(dpar, ddat,
						pos0, ddframe0, compute_xsec0, posn0, ndel0, ndop0,
						s, f, nfrm_alloc, hf);

				/* Switch to odd frame and switch to GPU1 */
				f++;	if (f==nfrm_alloc)	break;
				gpuErrchk(cudaSetDevice(GPU0));

				delay_params_mgpu_krnl<<<1,1,0,gpu1_stream[f]>>>(dpar, ddat,
						pos1, ddframe1, compute_xsec1, posn1, ndel1, ndop1,
						s, f, nfrm_alloc, hf);
				hf++;
			}
			...
			gpuErrchk(cudaSetDevice(GPU0));
			gpuErrchk(cudaMemcpyAsync(&hposn0, posn0, sizeof(int)*nfrm_half,
					cudaMemcpyDeviceToHost, gpu0_stream[0]));
			gpuErrchk(cudaMemcpyAsync(&hcomp_xsec0, compute_xsec0, sizeof(int)*
					nfrm_half, cudaMemcpyDeviceToHost, gpu0_stream[1]));
			gpuErrchk(cudaMemcpyAsync(&hndop0, ndop0, sizeof(int)*nfrm_half,
					cudaMemcpyDeviceToHost, gpu0_stream[2]));
			gpuErrchk(cudaMemcpyAsync(&hndel0, ndel0, sizeof(int)*nfrm_half,
					cudaMemcpyDeviceToHost, gpu0_stream[3]));

			/* Copy GPU1 stuff to host arrays, then back to GPU0 */
			gpuErrchk(cudaSetDevice(GPU1));
			gpuErrchk(cudaMemcpyAsync(&hposn1, posn1, sizeof(int)*nfrm_half,
					cudaMemcpyDeviceToHost, gpu0_stream[0]));
			gpuErrchk(cudaMemcpyAsync(&hcomp_xsec1, compute_xsec1, sizeof(int)*
					nfrm_half, cudaMemcpyDeviceToHost, gpu0_stream[1]));
			gpuErrchk(cudaMemcpyAsync(&hndop1, ndop1, sizeof(int)*nfrm_half,
					cudaMemcpyDeviceToHost, gpu0_stream[2]));
			gpuErrchk(cudaMemcpyAsync(&hndel1, ndel1, sizeof(int)*nfrm_half,
					cudaMemcpyDeviceToHost, gpu0_stream[3]));
			gpuErrchk(cudaSetDevice(GPU0));
			...
			for (int f=0; f<nfrm_alloc; f++) {
				...
				/* Switch back to even device for next iteration */
				gpuErrchk(cudaSetDevice(GPU0));
			}
			...
			for (int f=0; f<nfrm_alloc; f++) {
				...
				/* Even frame */
				int comp_xsec = hcomp_xsec0[hf];
				/* Start the if block for computing zmax and/or cross-section */
				if (hcomp_zmax[s] || comp_xsec) {
				...
				/* Increase half-frame counter and switch back to GPU0 */
				hf++;
				gpuErrchk(cudaSetDevice(GPU0));
			...
			/* Determine which POS pixels cover the target, and get distance
			 * toward Earth of each POS pixel. Pass the frame streams, too. */
			posvis_mgpu(dpar, dmod, ddat, pos, verts, orbit_offset,
					hposn, outbndarr, s, hnframes[s], 0, nf, 0, c, htype[s],
			...
			for (int f=0; f<nfrm_alloc; f++) {
				...
				if (hcomp_zmax[s] || comp_xsec) {
					 ...
					 * to create the fit image by mapping power from the plane
					 * of sky to delay-Doppler space.    				  */
					clrvect_krnl<<<ddBLK[f],THD, 0, gpu0_stream[hf]>>>(ddat,
							ddsize[f], s, f);
				}/* End frames loop again to call pos2deldop streams version */
				...
				if (hcomp_zmax[s] || comp_xsec) {
					 ...
					 * to create the fit image by mapping power from the plane
					 * of sky to delay-Doppler space.    				  */
					clrvect_krnl<<<ddBLK[f],THD, 0, gpu1_stream[hf]>>>(ddat,
							ddsize[f], s, f);
				}/* End frames loop again to call pos2deldop streams version */
			...
			/* Call the CUDA pos2deldop function */
			pos2deldop_gpu(dpar, dmod, ddat, pos, ddframe, xylim, ndel, ndop,
					0.0, 0.0, 0.0, 0, s, hnframes[s], 0, outbndarr, gpu0_stream);

			/* Calculate zmax for all frames (assumption: all pos in this set
			 * have the same pixel dimensions) */
			if (hcomp_zmax[s]) {
				zmax = compute_zmax_gpu(ddat, pos, nfrm_alloc, npxls[0], s, gpu0_stream);
				zmax_finalize_krnl<<<1,1>>>(zmax);
				checkErrorAfterKernelLaunch("zmax_finalize_streams2_krnl");
			}
			...
			/* Calculate radar cross section for each frame in set */
			xsec[0] = compute_deldop_xsec_gpu(ddat, hnframes[s], ddsize[0], s, gpu0_stream);
			compute_xsec_deldop_krnl<<<1,1>>>(xsec[0]);
			checkErrorAfterKernelLaunch("compute_xsec_final_streams2_krnl");
		...
		case DOPPLER:
			...
			for (f=0; f<nfrm_alloc; f++) {
				/* Start the if block for computing zmax and/or cross-section */
				if (hcomp_xsec[f]) {
					vpst_set_posmtrx_krnl<<<1,THD9,0,gpu0_stream[f]>>>(ddat, pos, htype[s],
							s, f);
					posclr_krnl<<<BLK[f],THD,0,gpu0_stream[f]>>>(pos,posn,f,0);
				}
			} checkErrorAfterKernelLaunch("vpst_set_posmtrx_streams_krnl and "
					"posclr_streams_krnl (Doppler)");
			...
			for (f=0; f<nfrm_alloc; f++) {
				if (hcomp_xsec[f]) {
					...
					clrvect_krnl<<<ddBLK[f],THD, 0, gpu0_stream[f]>>>(ddat,
							hndop[f], s, f);
					/* End frames loop again to call pos2deldop streams version */
				}
			} checkErrorAfterKernelLaunch("clrvect_streams_krnl");
			...
			pos2doppler_gpu(dpar, dmod, ddat, pos, dframe, xylim, 0.0,
					0.0, 0.0, ndop, 0, s, hnframes[s], 0, outbndarr, gpu0_stream);
			...
			for (f=0; f<nfrm_alloc; f++) {
				if (hcomp_xsec[f])
					compute_xsec_doppler_krnl<<<1,1,0,gpu0_stream[f]>>>(ddat, xsec[f], s, f);
				if (compute_cosdelta)
					compute_cosdelta_krnl<<<1,1,0,gpu0_stream[f]>>>(ddat, s, f);
			}
			break;
		case POS:
			break;
		case LGHTCRV:
			...
			if (hcomp_brightness[s]) {
				...
				/* Clear out the plane-of-sky first */
				for (f=1; f<nfrm_alloc; f++) {
					vpst_set_posmtrx_krnl<<<1,THD9,0,gpu0_stream[f-1]>>>(
							ddat, pos, htype[s], s, f);

					posclr_krnl<<<BLK[f],THD, 0, gpu0_stream[f-1]>>>(pos,posn,f,1);
				}
				checkErrorAfterKernelLaunch("vpst_set_posmtrx_streams_krnl and"
						"posclr_streams_krnl (Lightcurve) ");
				...
				if (bistatic_all)
					posvis_mgpu(dpar, dmod, ddat, pos, verts,
							orbit_offset, hposn, outbndarr, s, hnframes[s], 1,
							nf, 0, c, htype[s],	gpu0_stream, gpu1_stream);

				if (bistatic_all) {
					for (f=1; f<nfrm_alloc; f++) {
						/* Initialize this stream for the posmask kernel to follow */
						posmask_init_krnl<<<1,1,0,gpu0_stream[f-1]>>>(pos,
								so, pixels_per_km, f);

						/* Now call posmask kernel for this stream, then loop
						 * to next stream and repeat 	 */
						posmask_krnl<<<BLK[f],THD,0,gpu0_stream[f-1]>>>(
								dpar, pos, so, pixels_per_km, posn, npxls[f],
								lc_xspan[f], f);
				 ...
				 * ncalc-points total 					 */
				apply_photo_cuda_streams_f(dmod, ddat, pos, xylim, span, BLK, nThreadspx1,
							0, s, hnframes[s], npxls, gpu0_stream);
