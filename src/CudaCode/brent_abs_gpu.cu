		...
		int nf, cudaStream_t *bf_stream)
{
	...
	b = ((ax > cx) ? ax : cx);
	x = w = v = bx;
	fw = fv = fx = (*f)(x, verts, nviews, nsets, nf, bf_stream);
	...
	if (fabs(x - xm) <= (tol2 - 0.5 * (b - a))) {
		...
		u = (fabs(d) >= tol1 ? x+d : x+SIGN(tol1,d));
		fu = (*f)(u, verts, nviews, nsets, nf, bf_stream);
...
__host__ double brent_abs_gpu_dbg(double ax,double bx,double cx,double (*f)(double,
		struct vertices_t**, unsigned char*, unsigned char*, int*, int*, int*, int,
		int, cudaStream_t*), double tol,double abstol,double *xmin, struct
		vertices_t **verts, unsigned char *htype, unsigned char *dtype,
		int *nframes, int *nviews, int *lc_n, int nsets, int nf, cudaStream_t *bf_stream)
{
	...
	for (iter=1; iter<=ITMAX; iter++) {
		...
		if (fabs(x - xm) <= (tol2 - 0.5 * (b - a))) {
			...
			*xmin = x;
			printf("loops in brent_abs_gpu, %i\n", dbg);
			return fx;
		}
