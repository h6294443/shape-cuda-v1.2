...
__global__ void posvis_init_MFS_krnl64(
		struct par_t *dpar,
		struct pos_t **pos,
		double4 *ijminmax_overall,
		double3 *oa,
...
__global__ void posvis_facet_MFS_krnl64(
		struct pos_t **pos,
		struct vertices_t **verts,
		double4 *ijminmax_overall_gm,
		double3 orbit_offs,
...
//									}
//									if (frm==15 && i==-23 && j==-7 && !src) {
//										printf("facet %i at (%i, %i). z=%3.8g, old=%3.8g, n.z=%3.8g, cose[%i][%i]=%3.8g\n",
//												f, i, j, z, old, n.z, i, j, pos[frm]->cose[i][j]);
//										printf("src=%i, bistatic=%i, cosi[%i][%i]=%3.8g\n", src, bistatic, i, j, pos[frm]->cosi[i][j]);
//									}
//									/*else if (n.z<=0.0 && (temp>=z-tol) && (temp<=z+tol)) {
//										if (src)
//											atomicExch((unsigned long long int*)&pos[frm]->cosill[i][j],
//													__double_as_longlong(0.0));
//										else
...
//		atomicMax64(&ijminmax_overall_gm[frm].z, ijminmax_overall_sh.z);
//
//		if (frm==15&&blockIdx.x==0) {
//			printf("pos[15]->cose[-23][-7]=%3.8g\n", pos[15]->cose[-23][-7]);
//			printf("pos[15]->cosi[-23][-7]=%3.8g\n", pos[15]->cosi[-23][-7]);
//		}
//
