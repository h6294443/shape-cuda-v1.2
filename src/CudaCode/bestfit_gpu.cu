...
__device__ double bf_hotparamval, bf_dummyval=0.0;
		...
		struct dat_t *dat)
{
	...
	if (call_vary_params)
	{
		...
		if (FLOAT)
			realize_spin_gpu_f(dpar, dmod, ddat, htype, nframes,
		...
		cudaStream_t *bf_stream)
{
	...
	if (newspin) {
		if (FLOAT)
			realize_spin_gpu_f(sdev_par, sdev_mod, sdev_dat, htype, nframes,
					nviews, nsets, bf_stream);
		else
