...
__host__ double objective_MFS_gpu(double x, struct vertices_t **verts,
...
__host__ double bestfit_MFS_gpu(struct par_t *dpar, struct mod_t *dmod,
		...
		struct dat_t *dat)
{
	/* This bestfit routine is used for models consisting entirely of single-
	 * frame data sets. Instead of streaming frames, all MFS routines assume
	 * that each data set has only one single frame. This allows the sets to
	 * be streamed instead of frames. */
	char hostname[MAXLEN], dofstring[MAXLEN];
	int i, iter=0, p, cntr, first_fitpar, partype, keep_iterating=1, ilaw,
			nf, term_maxiter;
	long pid_long;
	...
	double beginerr, enderr, ax, bx, cx, obja, objb, objc, xmin, final_chi2,
		...
		*hfparabstol, objfunc_start, term_prec;
	unsigned char *htype, *dtype, action, avoid_badpos, term_badmodel;
	int nsets, *lc_n, *nviews, nfpar, *hfpartype, npar_update, max_frames=0,
			max_streams=0, *flags, *hflags;
	struct vertices_t **verts;
	dim3 THD, BLK;
	...
	/* The following check is necessary to ensure the dVdIdCOM reduction has
	 * enough streams to operate properly	 */
	if (nsets < 13) max_streams = 13;
	else	max_streams = nsets;

	/* Create streams for gpu0 (the only gpu in single-GPU mode) */
	cudaStream_t bf_stream[max_streams];
	for (int s=0; s<max_streams; s++)
		gpuErrchk(cudaStreamCreate(&bf_stream[s]));
	 ...
	 * frames for any one set) to device so that objective_gpu
	 * can retrieve it later */
	set_verts_shortcut_krnl<<<1,1>>>(dmod, verts, nsets);
	checkErrorAfterKernelLaunch("set_verts_shortcut_krnl");
...
//		return(0);
	...
	do {
		...
		/* Show breakdown of chi-square by data type    */
		chi2_MFS_gpu(dpar, ddat, 1, nsets, bf_stream);
...
//				printf("entered badpos if-block\n");
				...
				while (hflags[2]==1) {
					objective_MFS_gpu(hotparamval, verts, nviews, nsets,
					nf, bf_stream);
			...
			mnbrak_MFS_gpu(&ax, &bx, &cx, &obja, &objb, &objc,
					objective_MFS_gpu, verts, nviews, nsets, nf, bf_stream);
			/* Before homing in on local minimum, initialize flags that will
			 * tell us if model extended beyond POS frame (sky rendering) for
			 * any trial parameter value(s), if it extended beyond any POS ima-
			 * ges, and if it was too wide in delay-Doppler space         */
...
//			printf("xmin, %3.8g\n", xmin);
			...
			if ((newsize && vary_delcor0_size) || ((newshape || newspin) &&
					vary_delcor0_shapespin)) {
				deldop_zmax_save = deldop_zmax;
				realize_delcor_MFS_gpu(ddat, delta_delcor0, 2, nsets);  /* reset delcor0, then delcor0_save */
			} else if (newdelcor)
				realize_delcor_MFS_gpu(ddat, 0.0, 0, nsets);  /* set delcor0_save to delcor0 */

			if ((newspin && vary_dopscale_spin) || ((newsize || newshape) &&
					vary_dopscale_sizeshape)) {
				cos_subradarlat_save = cos_subradarlat;
				realize_dopscale_MFS_gpu(dpar, ddat, dopscale_factor, 2, nsets);  /* reset dopscale, then dopscale_save */
			} else if (newdopscale) {
				realize_dopscale_MFS_gpu(dpar, ddat, 1.0, 0, nsets);  /* set dopscale_save to dopscale */
			}
...
//			printf("bf_get_flags\n");
			...
			if (++cntr >= npar_update) {
				cntr = 0;
				//				showvals = 1;
				calc_fits_MFS_gpu(dpar, dmod, ddat, verts, nviews,
						nsets, nf, bf_stream);
...
//			write_dat( dpar, ddat);
	...
	/* Show final values of reduced chi-square, individual penalty functions,
	 * and the objective function  */
	final_chi2 = chi2_MFS_gpu(dpar, ddat, 1, nsets, bf_stream);
...
__host__ double objective_MFS_gpu(
		double x,
		struct vertices_t **verts,
		int *nviews,
		int nsets,
