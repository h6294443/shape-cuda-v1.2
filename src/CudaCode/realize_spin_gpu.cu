...
__global__ void realize_spin_dop_krnl2(struct mod_t *dmod, struct dat_t *ddat,
...
__global__ void realize_spin_deldop_krnl2(struct mod_t *dmod, struct dat_t *ddat,
...
__global__ void realize_spin_poset_krnl2(struct mod_t *dmod, struct dat_t *ddat,
		struct par_t *dpar, int nviews, int s, int size)
{
...
__global__ void realize_spin_lghtcrv_krnl2(struct mod_t *dmod, struct dat_t *ddat,
		struct par_t *dpar, int s, int size)
{
...
__host__ void realize_spin_gpu2(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		unsigned char *htype,
		...
		cudaStream_t *rs_stream)
{
	...
	for (s=0; s<nsets; s++) {
		...
		case DOPPLER:
			...
			/* Loop through every frame and launch a stream kernel with nview
			 * threads  */
			realize_spin_dop_krnl2<<<BLKfrm,THD64>>>(dmod, ddat, dpar, nviews[s],
					s, nframes[s]);
		...
		case DELAY:
			...
			/* Apply dataset's spin offsets (also in body coordinates)
			 * to the intrinsic spin vector of this view.                    */
			realize_spin_deldop_krnl2<<<BLKfrm,THD64>>>(dmod, ddat, dpar,
		...
		case POS:
			...
			/* Apply dataset's spin offsets (also in body coordinates)
			 * to the intrinsic spin vector of this view. */
			realize_spin_poset_krnl2<<<BLKfrm,THD64>>>(dmod, ddat, dpar,
					nviews[s], s, nframes[s]);
		...
		case LGHTCRV:
			...
			/* Apply this dataset's spin offsets (also in body coordinates)
				to the intrinsic spin vector of this point. */
			realize_spin_lghtcrv_krnl2<<<BLKfrm,THD64>>>(dmod, ddat, dpar, s,
					(nframes[s]+1)); // f = i, k = 0
