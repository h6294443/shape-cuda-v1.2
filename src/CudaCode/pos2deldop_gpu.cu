...
__global__ void pos2deldop_init_MFS_krnl(
		struct dat_t *ddat,
		struct deldopfrm_t **frame,
		int *idel0,
		int *idop0,
	...
	if (s < nsets) {
		/*  Initialize variables to avoid compilation warnings  */
		idel0[s] = idop0[s] = /*p2ds_any_overflow = */0;
		frame[s] = &ddat->set[s].desc.deldop.frame[0];
		frame[s]->idellim[0] = ndel[s] + 999999;
		frame[s]->idellim[1] = frame[s]->idoplim[1] = -999999;
		frame[s]->idoplim[0] = ndop[s] + 999999;
		frame[s]->dellim[0] =  HUGENUMBER;
		frame[s]->dellim[1] = -HUGENUMBER;
		frame[s]->doplim[0] =  HUGENUMBER;
		frame[s]->doplim[1] = -HUGENUMBER;

		badradararr[s] = 0;
		any_overflow[s] = 0;
		frame[s]->badradar_logfactor = 0.0;
	}
}
...
__global__ void pos2deldop_data_sampling_MFS_krnl(
	...
	if (s < nsets) {
		codemethod[s] = ddat->set[s].desc.deldop.codemethod;
		spb[s] = ddat->set[s].desc.deldop.spb;
		stride[s] = ddat->set[s].desc.deldop.stride;
		dopfftlen[s] = ddat->set[s].desc.deldop.dopfftlen;
		spb_over_stride[s] = spb[s]/stride[s];
		one_over_spb[s] = 1.0/spb[s];
		spb_sq[s] = spb[s]*spb[s];
		if (codemethod[s] != LONG_ORIG) {
			const1[s] = (3*spb[s] - 1)/(2.0*stride[s]);
			const2[s] = (spb[s] - 1)/(2.0*stride[s]);
		} else {
			const1[s] = (double) spb_over_stride[s];
			const2[s] = 0.0;  /* not used with this code + reduction method */
		}
		/*  Converts from km towards radar to delay bins  */
		delfact[s] = -KM2US/ddat->set[s].desc.deldop.del_per_pixel;

		dopfact[s] = ddat->set[s].desc.deldop.dopscale.val * KM2HZFACT *
				pos[s]->km_per_pixel * ddat->set[s].desc.deldop.Ftx /
				ddat->set[s].desc.deldop.dop_per_pixel;
		dop[s].y = frame[s]->dopDC_vig;
		/*  Get w, the apparent spin vector in observer coordinates  */
		dev_cotrans5(w, frame[s]->view[v].oe, frame[s]->view[v].spin, 1, s);
		 ...
		 * increment (Hz) per km perpendicular to the projected spin axis in
		 * the plane of the sky.     */
		axay[s].x = -w[s].y * dopfact[s];
		axay[s].y =  w[s].x * dopfact[s];
		frame[s]->view[v].km2Hz = sqrt(axay[s].x*axay[s].x+axay[s].y*axay[s].y) *
				ddat->set[s].desc.deldop.dop_per_pixel / pos[s]->km_per_pixel;

		/* Compute the absolute value of the difference between the maximum (or minimum)
		 * Doppler on any given POS pixel's edge and the Doppler at its center             */
		if (w[s].x != 0.0 || w[s].y != 0.0)
			dop[s].z = frame[s]->view[v].km2Hz * 0.5 * pos[s]->km_per_pixel
			* sqrt(w[s].x * w[s].x + w[s].y * w[s].y) /	MAX( fabs(w[s].x),
					fabs(w[s].y));
		else
			dop[s].z = 0.0;
		 ...
		 *	dop.y - dopDC_vig
		 * 	dop.z - dop_extra		 */
		xyincr[s].x = axay[s].x / dpar->nsinc2;
		xyincr[s].y = axay[s].y / dpar->nsinc2;
		dop[s].w = -(dpar->nsinc2 - 1) * (xyincr[s].x + xyincr[s].y) / 2;
		dop[s].x = (dpar->nsinc2 - 1) * (fabs(xyincr[s].x) + fabs(xyincr[s].y)) / 2;

		if (2 * dop[s].x + dpar->sinc2width + 1 > MAXBINS) {
			badradararr[s] = 1;
			frame[s]->badradar_logfactor += log(
					(2 * dop[s].x + dpar->sinc2width + 1) / MAXBINS);
			if (dpar->warn_badradar) {
				printf(
						"\nWARNING in pos2deldop_cuda_MFS.c for set %2d\n",
						s);
				printf(
						"        sinc^2 function evaluated at %d Doppler bins, must be <= %d\n",
						(int) ceil(2 * dop[s].x + dpar->sinc2width + 1),
						MAXBINS);
			}
		}
		/* Get the COM delay and Doppler bins, corrected for ephemeris drift and adjusted for
		 ...
		 * posvis routine has already adjusted the "z" values for all POS pixels), whereas the
		 * Doppler adjustment for orbital motion must be done here explicitly.                  */
		deldopshift[s].x = frame[s]->delcom_vig + frame[s]->view[v].deloff;
		deldopshift[s].y = frame[s]->dopcom_vig + frame[s]->view[v].dopoff
				+ orbit_dopoff;
		...
		/* Copy frame[frm]->dellim[2] and frame[frm]->doplim[2] to the device variables */
		deldoplim[s].w = frame[s]->dellim[0];
		deldoplim[s].x = frame[s]->dellim[1];
		deldoplim[s].y = frame[s]->doplim[0];
		deldoplim[s].z = frame[s]->doplim[1];
	}
}
...
__global__ void pos2deldop_pixel_MFS_krnl(
		...
		double *delfact,
		double *dopfact) {
	/* This version is functionally identical to the 64moda version, however:
     * Each data set is assumed to have only one single frame.  The sets are
     * streamed instead of the frames. */
	...
	if (threadIdx.x==0) {
		...
		nsinc2 = dpar->nsinc2;
		sinc2width = dpar->sinc2width;
		km_per_pixel = pos[s]->km_per_pixel;
	...
	if (offset < nThreads) {
		...
		if (cose > 0.0 ) {
			 ...
			 * span multiple bins we'll err on the side of computing more
			 * contributions rather          than fewer.       */
			delPOS = pos[s]->z[x][y] * delfact[s] + deldopshift[s].x;
			idel_min = (int) floor(delPOS - const1[s]) + 1;
			idel_max = (int) ceil(delPOS + const1[s]) - 1;
			dopPOS = axay[s].x*(x - orbit_xoff) + axay[s].y*
					(y - orbit_yoff) + deldopshift[s].y;
			idop_min = (int) floor(dopPOS - dop[s].x + 1 - sinc2width/2.0);
			idop_max = (int) floor(dopPOS + dop[s].x + sinc2width/2.0);

			/*  For the short code, sensitivity drops as we move away from DC. (This variation is slow,
			 *  so we can just evaluate the response at the center of the POS pixel.)
			 *  Note that the SINC2 macro multiplies its argument by pi.        */
			codefactor = (codemethod[s] == SHORT) ? SINC2( (dopPOS-dop[s].y)/dopfftlen[s] ) : 1.0;
			...
			/*  Check whether or not all delay-Doppler pixels which will receive power from this POS pixel
			 *  fall within the data frame; if not, initialize the "overflow" image if necessary.         */
			if ((idel_min>=1) && (idel_max<=ndel[s]) && (idop_min>=1) &&
					(idop_max<=ndop[s]))
				in_bounds = 1;
			else {
				in_bounds = 0;
				if (!any_overflow[s])
					atomicExch(&any_overflow[s], 1);
			}
				...
				if (codemethod[s] != LONG_ORIG) {
					...
					default:
						...
						m_max = MIN( (int) ceil((delPOS - idel + const1[s])
								* stride[s]) , spb[s] ) - 1;
						arg_sample = (delPOS - (idel - const2[s])) /
								spb_over_stride[s] - m_min*one_over_spb[s];
						for (m=m_min; m<=m_max; m++) {
							del_contribution[idel-idel_min] += TRI( arg_sample );
							arg_sample -= one_over_spb[s];
						}
						del_contribution[idel-idel_min] *=
								del_contribution[idel-idel_min]/spb_sq[s];
						break;
					}
				} else {
			...
			for (idop=idop_min; idop<=idop_max; idop++) {
				...
				case 1:
					...
					break;
				case 2:
					arg_bl = dopPOS + dop[s].w - idop;   /* bl = bottom left */
					sinc2_mean = ( SINC2( arg_bl ) + SINC2( arg_bl+xincr) +
							SINC2( arg_bl+yincr) + SINC2( arg_bl+xincr+yincr) ) / 4;
					break;
				default:
					arg_left = dopPOS + dop[s].w - idop;
					sinc2_mean = 0.0;
					for (i=0; i<nsinc2; i++) {
						sinc2arg = arg_left;
						for (j=0; j<nsinc2; j++) {
			 ...
			 * account for the fact that the POS pixel area is projected area
			 * rather than physical area on the target surface.      */
			amp = dev_radlaw_mod( &dmod->photo, ddat->set[s].desc.deldop.iradlaw,
					cose) * km_per_pixel * km_per_pixel * codefactor / sumweights;
			...
			if (in_bounds) {
				...
				for (idel=idel_min; idel<=idel_max; idel++)
					for (idop=idop_min; idop<=idop_max; idop++) {
						...
						fit_contribution = amp * del_contribution[idel-idel_min]
	                                          * dop_contribution[k];
						atomicAdd(&frame[s]->fit[idel][idop], fit_contribution);
					}
			}
		}
	}
	...
	if (threadIdx.x==0)	{	/* So thread 0 for every block */
		/*  Update rectangular delay-Doppler region (row/column numbers) with !0 power according to model  */
		atomicMin(&frame[s]->idellim[0], idellim[0]);
		atomicMax(&frame[s]->idellim[1], idellim[1]);
		atomicMin(&frame[s]->idoplim[0], idoplim[0]);
		atomicMax(&frame[s]->idoplim[1], idoplim[1]);

		atomicMin64(&deldoplim[s].w, deldoplim_sh.w);
		atomicMax64(&deldoplim[s].x, deldoplim_sh.x);
		atomicMin64(&deldoplim[s].y, deldoplim_sh.y);
		atomicMax64(&deldoplim[s].z, deldoplim_sh.z);
	}
}
...
__global__ void pos2deldop_deldoplim_MFS_krnl(
	...
	int s = blockIdx.x * blockDim.x + threadIdx.x;
	double dlppxl, dpppxl;
	dlppxl = ddat->set[s].desc.deldop.del_per_pixel;
	dpppxl = ddat->set[s].desc.deldop.dop_per_pixel;

	if (s<nsets) {
		frame[s]->dellim[0] = deldoplim[s].w;
		frame[s]->dellim[1] = deldoplim[s].x;
		frame[s]->doplim[0] = deldoplim[s].y;
		frame[s]->doplim[1] = deldoplim[s].z;

		/*  Convert the model's floating-point delay-Doppler limits from floating-
		 *  point row and column numbers to usec and Hz, and widen the Doppler limits
		 *  to account for nonzero POS pixel width  */
		frame[s]->dellim[0] = (frame[s]->dellim[0] - deldopshift[s].x)*dlppxl;
		frame[s]->dellim[1] = (frame[s]->dellim[1] - deldopshift[s].x)*dlppxl;
		frame[s]->doplim[0] = (frame[s]->doplim[0] - deldopshift[s].y)*dpppxl
				- dop[s].z;
		frame[s]->doplim[1] = (frame[s]->doplim[1] - deldopshift[s].y)*dpppxl
				+ dop[s].z;
...
__global__ void pos2deldop_overflow_MFS_krnl(
	...
	if (s < nsets) {
		 ...
		 *  Also compute the summed cross section and the mean delay and Doppler
		 *  bins for the overflow region, for use with the "delcorinit" action    */
		frame[s]->overflow_o2 = 0.0;
		frame[s]->overflow_m2 = 0.0;
		frame[s]->overflow_xsec = 0.0;
		frame[s]->overflow_delmean = 0.0;
		frame[s]->overflow_dopmean = 0.0;
...
__host__ void pos2deldop_MFS_gpu(
		...
		cudaStream_t *p2d_stream)
{
	int xspan[nsets], yspan, nThreads[nsets], *idop0, *idel0,
	*any_overflow, *codemethod, *spb, *stride, *dopfftlen, *spb_over_stride,
	*spb_sq;
	double *one_over_spb, *const1, *const2, *delfact, *dopfact;
	double2 *axay, *xyincr, *deldopshift;
	double3 *w;
	double4 *dop, *deldoplim;
	int4 host_xylim[nsets];
	dim3 BLK[nsets], THD, BLKsets, THDsets;
	THD.x = maxThreadsPerBlock;	THDsets.x = nsets;
	BLKsets.x = floor((THDsets.x - 1 + nsets)/THDsets.x);

	gpuErrchk(cudaMalloc((void**)&any_overflow, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&idop0, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&idel0, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&codemethod, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&spb, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&stride, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&dopfftlen, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&spb_over_stride, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&spb_sq, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&any_overflow, sizeof(int)*nsets));
	gpuErrchk(cudaMalloc((void**)&one_over_spb, sizeof(double)*nsets));
	gpuErrchk(cudaMalloc((void**)&const1, sizeof(double)*nsets));
	gpuErrchk(cudaMalloc((void**)&const2, sizeof(double)*nsets));
	gpuErrchk(cudaMalloc((void**)&delfact, sizeof(double)*nsets));
	gpuErrchk(cudaMalloc((void**)&dopfact, sizeof(double)*nsets));
	gpuErrchk(cudaMalloc((void**)&w, sizeof(double3)*nsets));
	gpuErrchk(cudaMalloc((void**)&axay, sizeof(double2)*nsets));
	gpuErrchk(cudaMalloc((void**)&xyincr, sizeof(double2)*nsets));
	gpuErrchk(cudaMalloc((void**)&deldopshift, sizeof(double2)*nsets));
	gpuErrchk(cudaMalloc((void**)&deldoplim, sizeof(double4)*nsets));
	gpuErrchk(cudaMalloc((void**)&dop, sizeof(double4)*nsets));
	...
	for (int s=0; s<nsets; s++) {
		xspan[s] = host_xylim[s].x - host_xylim[s].w + 1;
		yspan = host_xylim[s].z - host_xylim[s].y + 1;
		nThreads[s] = xspan[s]*yspan;
		BLK[s].x = floor((THD.x -1 + nThreads[s]) / THD.x);
	}
	...
	cudaFree(stride);
	cudaFree(dopfftlen);
	cudaFree(spb_over_stride);
	cudaFree(spb_sq);
	cudaFree(one_over_spb);
	cudaFree(const1);
	cudaFree(const2);
	cudaFree(delfact);
	cudaFree(dopfact);
	cudaFree(any_overflow);
}
