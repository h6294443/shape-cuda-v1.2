...
__global__ void pos2deldop_data_sampling_MFS_krnl64(
	...
	if (f < size) {
		codemethod[f] = ddat->set[set].desc.deldop.codemethod;
		spb[f] = ddat->set[set].desc.deldop.spb;
		stride[f] = ddat->set[set].desc.deldop.stride;
		dopfftlen[f] = ddat->set[set].desc.deldop.dopfftlen;
		spb_over_stride[f] = spb[f]/stride[f];
		one_over_spb[f] = 1.0/spb[f];
		spb_sq[f] = spb[f]*spb[f];
		if (codemethod[f] != LONG_ORIG) {
			const1[f] = (3*spb[f] - 1)/(2.0*stride[f]);
			const2[f] = (spb[f] - 1)/(2.0*stride[f]);
		} else {
			const1[f] = (double) spb_over_stride[f];
			const2[f] = 0.0;  /* not used with this code + reduction method */
		}
		/*  Converts from km towards radar to delay bins  */
		delfact[f] = -KM2US/ddat->set[set].desc.deldop.frame[f].del_per_pixel;

		dopfact[f] = ddat->set[set].desc.deldop.frame[f].dopscale.val * KM2HZFACT *
				pos[0]->km_per_pixel * ddat->set[set].desc.deldop.frame[f].Ftx /
				ddat->set[set].desc.deldop.frame[f].dop_per_pixel;
		dop[f].y = frame[f]->dopDC_vig;
		/*  Get w, the apparent spin vector in observer coordinates  */
		dev_cotrans5(w, frame[f]->view[v].oe, frame[f]->view[v].spin, 1, f);
		 ...
		 * increment (Hz) per km perpendicular to the projected spin axis in
		 * the plane of the sky.     */
		axay[f].x = -w[f].y * dopfact[f];
		axay[f].y =  w[f].x * dopfact[f];
		frame[f]->view[v].km2Hz = sqrt(axay[f].x*axay[f].x+axay[f].y*axay[f].y) *
				ddat->set[set].desc.deldop.frame[f].dop_per_pixel / pos[f]->km_per_pixel;
		...
		cudaStream_t *p2d_stream)
{
	...
	*spb_sq;
	...
	gpuErrchk(cudaMalloc((void**)&idop0, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&idel0, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&codemethod, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&spb, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&stride, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&dopfftlen, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&spb_over_stride, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&spb_sq, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&any_overflow, sizeof(int)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&one_over_spb, sizeof(double)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&const1, sizeof(double)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&const2, sizeof(double)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&delfact, sizeof(double)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&dopfact, sizeof(double)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&w, sizeof(double3)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&axay, sizeof(double2)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&xyincr, sizeof(double2)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&deldopshift, sizeof(double2)*nfrm_alloc));
	...
	checkErrorAfterKernelLaunch("pos2deldop_init_krnl2");
	/* Launch kernel to determine data sampling/radar parameters.  */
	pos2deldop_data_sampling_MFS_krnl64<<<BLKfrm,THD64>>>(dpar, ddat, frame, pos,
			axay, xyincr, deldopshift, w, dop, deldoplim, xylim, set,
			nfrm_alloc, v, orbit_dopoff, badradararr, codemethod, spb, stride,
			dopfftlen, spb_over_stride, spb_sq, one_over_spb, const1, const2,
			delfact, dopfact);
	checkErrorAfterKernelLaunch("pos2deldop_data_sampling_krnl64");
	gpuErrchk(cudaMemcpy(host_xylim, xylim, nfrm_alloc*sizeof(int4), cudaMemcpyDeviceToHost));
