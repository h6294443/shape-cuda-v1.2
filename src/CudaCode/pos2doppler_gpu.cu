...
__global__ void pos2doppler_init_krnl2(
		struct dat_t *ddat,
		struct dopfrm_t **frame,
		struct pos_t **pos,
		int4 *xylim,
...
__global__ void pos2doppler_radar_parameters_f_krnl(
		struct par_t *dpar,
		struct dat_t *ddat,
		struct dopfrm_t **frame,
		struct pos_t **pos,
	...
	/* single-threaded kernel, intended to be streamed */
	float dopfact;
	__shared__ int dpb, nsinc2, sinc2width;
	__shared__ double kmppxl;
	...
	if (threadIdx.x == 0) {
      ...
      km perpendicular to the projected spin axis in the plane of the sky.     */
		...
		dopfact = __double2float_rn(ddat->set[set].desc.doppler.dopscale.val) *
				KM2HZFACT * kmppxl *
				__double2float_rn(ddat->set[set].desc.doppler.Ftx) / dpb;
		axay[f].x = -w[f].y*dopfact;
		axay[f].y =  w[f].x*dopfact;
		frame[f]->view[v].km2Hz = sqrt(axay[f].x*axay[f].x +
				axay[f].y*axay[f].y) * dpb	/ kmppxl;
...
__global__ void pos2doppler_radar_parameters_krnl2(
		struct par_t *dpar,
		struct dat_t *ddat,
		struct dopfrm_t **frame,
		struct pos_t **pos,
...
__global__ void pos2doppler_pixel_f_krnl(
		struct par_t *dpar,
		struct mod_t *dmod,
		struct dat_t *ddat,
		struct pos_t **pos,
	...
	int idop, idop_min, idop_max, idop1, idop2, i, j, c, fac, k, zaddr;
	float tmp, amp, arg_left, sinc2arg, sinc2_mean, arg_bl, fit_contribution,
	sumweights, dop_contribution[MAXBINS], dopPOS;


	/* Set up the shared variables which are valid per warp */
	if (threadIdx.x == 0) {
	...
	if (offset < frame_size) {
		...
		if (pos[f]->cose_s[zaddr] > 0.0 && pos[f]->body[x][y] == body) {
			 ...
			 * dop.y - dopDC_vig
			 * dop.z - dop_extra		 */
			dopPOS = axay[f].x*(x - orbit_xyoff.x) + axay[f].y*(y - orbit_xyoff.y) + dopshift[f];
			idop_min = (int) floor(dopPOS - dop[f].x + 1 - sinc2width/2.0);
			idop_max = (int) floor(dopPOS + dop[f].x + sinc2width/2.0);
			 ...
			 * account for the fact that the POS pixel area is projected area
			 * rather than physical area on the target surface.	 */
			amp = dev_radlaw_f(&dmod->photo, ddat->set[set].desc.doppler.iradlaw,
					pos[f]->cose_s[zaddr], pos[f]->comp[x][y], pos[f]->f[x][y])
		    		 * kmpxl * kmpxl / sumweights;
...
__global__ void pos2doppler_finish_f_krnl(
		struct par_t *dpar,
		struct dat_t *ddat,
		struct dopfrm_t **frame,
		float4 *dop,
	...
	/* Single-threaded kernel, but intended to be streamed */
	int j, j1, j2;
	float lookfact, sdev_sq, variance, dopfactor;
	__shared__ float sdev, nlooks;
...
__global__ void pos2doppler_finish_krnl2(
		struct par_t *dpar,
		struct dat_t *ddat,
		struct dopfrm_t **frame,
		float4 *dop,
...
{
	int badradar, xspan[nframes], yspan, nThreads[nframes], f, *idop0;
	...
	gpuErrchk(cudaMalloc((void**)&dopshift, sizeof(float)*nframes));
	gpuErrchk(cudaMalloc((void**)&axay, 	sizeof(float2)*nframes));
	gpuErrchk(cudaMalloc((void**)&xyincr,   sizeof(float2)*nframes));
	gpuErrchk(cudaMalloc((void**)&doplim,   sizeof(float2)*nframes));
	...
	for (f=0; f<nframes; f++) {
		...
		pos2doppler_init_krnl<<<1,1,0,pds_stream[f]>>>(ddat, frame,
				pos, xylim,	w, doplim, ndop, idop0, set, v, nframes, f, badradararr);

		pos2doppler_radar_parameters_krnl<<<1,1,0,pds_stream[f]>>>(dpar,
	...
	for (f=0; f<nframes; f++) {
		...
		pos2doppler_pixel_krnl<<<BLK[f],THD,0, pds_stream[f]>>>(dpar,dmod,ddat,
				...
				xspan[f], set, nframes, nThreads[f], body, orbit_xoff, orbit_yoff, f);

		/* Launch the single-thread kernel to finish up Doppler calculations */
		pos2doppler_finish_krnl<<<1,1,0,pds_stream[f]>>>(dpar,ddat,frame,
		...
		cudaStream_t *pds_stream)
{
	...
	gpuErrchk(cudaMalloc((void**)&dopshift, sizeof(float)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&axay, 	sizeof(float2)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&xyincr,   sizeof(float2)*nfrm_alloc));
	gpuErrchk(cudaMalloc((void**)&doplim,   sizeof(float2)*nfrm_alloc));
	...
	pos2doppler_init_krnl2<<<BLKfrm,THD64>>>(ddat, frame, pos, xylim, w, doplim,
			ndop, idop0, set, v, nfrm_alloc, badradararr);
	checkErrorAfterKernelLaunch("pos2doppler_init_krnl2 in pos2doppler_gpu2");

	pos2doppler_radar_parameters_krnl2<<<BLKfrm,THD64>>>(dpar,
			ddat, frame, pos, dop, w, axay, xyincr, dopshift, orbit_dopoff,
			set, v, nfrm_alloc, badradararr);
	checkErrorAfterKernelLaunch("pos2doppler_radar_parameters_krnl2 in"
			"in pos2doppler_gpu.cu");
	...
	checkErrorAfterKernelLaunch("pos2doppler_pixel_krnl in pos2doppler_gpu2");

	/* Finish up Doppler calculations */
	pos2doppler_finish_krnl2<<<BLKfrm,THD64>>>(dpar,ddat,frame,
			dop, doplim, dopshift, idop0, ndop, set, nfrm_alloc, badradararr);
	checkErrorAfterKernelLaunch("pos2doppler_finish_krnl2 in pos2doppler_gpu2");
	gpuErrchk(cudaMemcpyFromSymbol(&badradar, pds_badradar_global, sizeof(int),
			0, cudaMemcpyDeviceToHost));
